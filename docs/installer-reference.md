# Installer automation and testing reference

These instructions were moved from the ISO README. Run examples from the repository root, not this documentation directory. Use disposable test targets and protect credentials. These are tool instructions, not claims that every scenario has passed on the current candidate. Start with the [ISO README](../README.md) and [agent guardrails](../AGENTS.md).
